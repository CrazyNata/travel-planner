...
function OverviewEditor({ trip, onUpdateTrip, onClose }: { trip: TripSummary; onUpdateTrip: (trip: TripSummary) => void; onClose: () => void }) {
  ...
  const save = () => {
    ...
    const nextPhotos = selectedPhoto ? [...existingPhotos.filter((photo) => photo.city !== city), { id: crypto.randomUUID(), image: selectedPhoto, city, description: caption }] : existingPhotos;
    onUpdateTrip({ ...trip, cities: weatherCities.join(", "), coverImage: nextPhotos[0]?.image, coverPhotos: nextPhotos });
    onClose();
  };
