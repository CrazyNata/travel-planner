...
function Sights({ sights, days, defaultCity, onToggle, onAdd }: { sights: StoredSight[]; days: DraftDay[]; defaultCity?: string; onToggle: (id: string) => void; onAdd: (sight: StoredSight) => void }) {
  ...
  return (
    <div>
      ...
      <main className="workspace">
        ...
        {tab === "sights" && <Sights sights={trip.sights || []} days={draftDays} defaultCity={trip.cities.split(",")[0]?.trim()} onToggle={(id) => onUpdateTrip({ ...trip, sights: trip.sights?.map((sight) => sight.id === id ? { ...sight, done: !sight.done } : sight) || [] })} onAdd={(sight) => onUpdateTrip({ ...trip, sights: [...(trip.sights || []), sight] })} />}
