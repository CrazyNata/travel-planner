...
function OverviewEditor({ trip, onUpdateTrip, onClose }: { trip: TripSummary; onUpdateTrip: (trip: TripSummary) => void; onClose: () => void }) {
  ...
  const [activePhotoId, setActivePhotoId] = useState<string | null>(() => trip.coverPhotos?.[0]?.id || (trip.coverImage ? "legacy-cover" : null));
  ...
  const selectPhotos = (files?: FileList | File[] | null) => {
    ...
    void Promise.all(images.map(compressCoverPhoto)).then((images) => {
      const photos = images.map((image) => ({ id: crypto.randomUUID(), image, city: routeCities[0] || "", description: "", textColor }));
  ...
  const save = () => {
    onUpdateTrip({ ...trip, cities: weatherCities.join(", "), coverImage: savedPhotos[0]?.image, coverPhotos: savedPhotos, coverTextColor: textColor, overviewMapPoints: mapPoints });
    onClose();
  };
    ...
    <div className="overview-editor-backdrop">
      <section className="overview-editor" role="dialog" aria-modal="true" aria-labelledby="overview-editor-title" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="overview-editor-content">
          <div className={`editor-photo-drop ${activePhoto ? "has-photo" : ""}`} style={activePhoto ? { backgroundImage: `linear-gradient(#11182733, #11182733), url(${activePhoto.image})` } : undefined} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); selectPhotos(event.dataTransfer.files); }}><input ref={input} type="file" accept="image/*" multiple onChange={(event) => { selectPhotos(event.target.files); event.target.value = ""; }} />{activePhoto ? <span>Выбранный слайд</span> : <><svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><rect x="3" y="3" width="18" height="18" rx="2" /><circle cx="8.5" cy="8.5" r="1.5" /><path d="m4 18 5-5 3 3 3-3 5 5" /></svg><span>Перетащите фото города</span><small>or <button type="button" onClick={() => input.current?.click()}>browse files</button></small></>}</div>
          <p className="editor-photo-hint">Выберите миниатюру, чтобы указать город и факт о ней.</p>
          <div className="editor-photo-list">{savedPhotos.map((photo, index) => <div className={`${draggedPhoto === index ? "dragging " : ""}${activePhotoId === photo.id ? "active" : ""}`} style={{ backgroundImage: `url(${photo.image})` }} title="Нажмите, чтобы редактировать; перетащите, чтобы изменить порядок" draggable onClick={() => setActivePhotoId(photo.id)} onDragStart={() => setDraggedPhoto(index)} onDragEnd={() => setDraggedPhoto(null)} onDragOver={(event) => event.preventDefault()} onDrop={() => { if (draggedPhoto === null || draggedPhoto === index) return; setSavedPhotos((photos) => { const next = [...photos]; const [moved] = next.splice(draggedPhoto, 1); next.splice(index, 0, moved); return next; }); setDraggedPhoto(null); }} key={photo.id}><button type="button" aria-label="Удалить фото" onClick={(event) => { event.stopPropagation(); setSavedPhotos((photos) => photos.filter((item) => item.id !== photo.id)); if (activePhotoId === photo.id) setActivePhotoId(null); }}>×</button></div>)}<button type="button" className="editor-add-photos" onClick={() => input.current?.click()}>＋ Добавить несколько фото</button></div>
          <input className="editor-field" value={activePhoto?.city || ""} onChange={(event) => updateActivePhoto({ city: event.target.value })} placeholder="Город" disabled={!activePhoto} />
          <textarea className="editor-caption" value={activePhoto?.description || ""} onChange={(event) => updateActivePhoto({ description: event.target.value })} placeholder="Факт о городе" disabled={!activePhoto} />
          <div className="editor-colors"><b>Цвет текста на фото</b><span>{colors.map((color) => <button type="button" className={(activePhoto?.textColor || textColor) === color ? "active" : ""} style={{ background: color }} onClick={() => updateActivePhoto({ textColor: color })} aria-label={`Выбрать цвет ${color}`} key={color} disabled={!activePhoto} />)}</span></div>
...
function WeatherOverview({ coverPhotos = weatherCoverPhotos }: { coverPhotos?: CoverPhoto[] }) {
  ...
  return <section className="weather-overview">
    ...
    <div className="weather-grid">
      {winterCities.map((city) => {
        ...
        const photo = coverPhotos.find((item) => item.city?.trim().toLocaleLowerCase("ru") === city.name.toLocaleLowerCase("ru"));
        return <article className={photo ? "weather-card has-photo" : "weather-card"} style={photo ? { backgroundImage: `linear-gradient(rgba(18, 18, 26, 0.42), rgba(18, 18, 26, 0.72)), url(${photo.image})` } : undefined} key={city.name}><h3>{city.name}</h3>{mode === "now" ? failed ? <p>Не удалось обновить погоду</p> : current ? <><b>{Math.round(current.temperature)}°C</b><span>{weatherDescription(current.code)}</span></> : <p>Обновляем...</p> : <><b>19 дек - 3 янв</b><span>Прогноз появится позже</span></>}</article>;
      })}
    </div>
  </section>;
}
