...
function RouteTab({ isDraft = false, draftDays = [], onAddDraftDay, onUpdateDraftDay }: { isDraft?: boolean; draftDays?: DraftDay[]; onAddDraftDay?: () => void; onUpdateDraftDay?: (day: number, changes: Partial<DraftDay>) => void }) {
  ...
  if (isDraft) return <><div className="route-toolbar"><span>Начните планирование: добавьте дни и автопереезды</span></div><div className="route-layout draft-route"><div className="day-rail">{draftDays.map((draftDay, index) => <button className={index === day ? "active" : ""} onClick={() => { setDay(index); setEditingRoadLeg(false); }} key={draftDay.id}><small>День {index + 1}</small><b>Новый маршрут</b><span>Даты не выбраны · {draftDay.places.length} мест</span></button>)}<button className="add-day" onClick={() => { setDay(draftDays.length); setEditingRoadLeg(false); onAddDraftDay?.(); }}>＋ Добавить день</button></div><section className="day-plan"><header><h2>{currentDraftDay.places.length || currentDraftDay.roadLeg ? `День ${day + 1} · Новый маршрут` : "Маршрут пока пуст"}</h2><span>{currentDraftDay.places.length ? `${currentDraftDay.places.length} мест` : "Добавьте автомобильный маршрут"}</span></header>{editingRoadLeg ? <RoadLegEditor roadLeg={currentDraftDay.roadLeg} onSave={(roadLeg) => { onUpdateDraftDay?.(day, { roadLeg }); setEditingRoadLeg(false); }} onCancel={() => setEditingRoadLeg(false)} /> : currentDraftDay.roadLeg ? <article className="road-leg-card"><span>АВТОМОБИЛЬНЫЙ МАРШРУТ</span><h3>{currentDraftDay.roadLeg.from} <b>→</b> {currentDraftDay.roadLeg.to}</h3><p>{currentDraftDay.roadLeg.checkIn ? `Заселение в отель: ${currentDraftDay.roadLeg.checkIn}` : "Время заселения не указано"}</p>{currentDraftDay.roadLeg.notes && <p className="road-leg-notes">{currentDraftDay.roadLeg.notes}</p>}<div>{currentDraftDay.roadLeg.avoidTolls && <small>Без платных дорог</small>}<a href={mapsUrl} target="_blank" rel="noreferrer">Открыть в Google Maps ↗</a><button type="button" onClick={() => setEditingRoadLeg(true)}>Изменить</button></div></article> : <button className="add-road-leg" onClick={() => setEditingRoadLeg(true)}>＋ Добавить автомобильный маршрут</button>}{currentDraftDay.places.length ? currentDraftDay.places.map((place, index) => <PlaceRow place={place} index={index} key={`${place}-${index}`} />) : null}</section><aside className="map-card"><TripMap /><footer><span>Маршрут дня</span><b>{currentDraftDay.places.length} точек</b></footer></aside></div></>;
