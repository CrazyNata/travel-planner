...
function OverviewEditor({ trip, onUpdateTrip, onClose }: { trip: TripSummary; onUpdateTrip: (trip: TripSummary) => void; onClose: () => void }) {
  ...
  const save = () => {
    const nextPhotos = [...savedPhotos, ...selectedPhotos.map((image) => ({ id: crypto.randomUUID(), image, city: slideCity.trim(), description: caption.trim(), textColor }))];
    ...
    <div className="overview-editor-backdrop" onClick={onClose}>
      <section className="overview-editor" role="dialog" aria-modal="true" aria-labelledby="overview-editor-title" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="overview-editor-content">
          ...
          <input className="editor-field" value={slideCity} onChange={(event) => setSlideCity(event.target.value)} placeholder="Город" />
          <textarea className="editor-caption" value={caption} onChange={(event) => setCaption(event.target.value)} placeholder="Факт о городе" />
