...
function CancellationPage({ onShowList, onAdd }: { onShowList: () => void; onAdd: () => void }) {
  ...
  return <section className="accommodation-page cancellation-page"><header className="accommodation-heading"><h2>Жильё</h2><button className="accent" onClick={onAdd}>＋ Добавить жильё</button></header><div className="accommodation-tabs"><button onClick={onShowList}>Список жилья</button><button className="active">Отмена</button></div><div className="cancellation-intro"><p>Сроки бесплатной отмены по каждому жилью — по возрастанию срочности.</p><button>↕ Сначала ближайшие</button></div><div className="cancellation-summary"><article><b>8</b><span className="free">● Бесплатно ещё</span></article><article><b>0</b><span className="soon">● Скоро платно</span></article><article><b>0</b><span className="paid">● Уже платно</span></article></div><div className="cancellation-list">{cancellations.map((stay) => <article key={stay.name}><div><h3>{cityFlag(stay.city)} {stay.name}</h3><p>{stay.city} · {stay.dates}</p></div><div className="cancellation-deadline"><b>{stay.days}<small>дн.</small></b><span>{stay.deadline}</span></div><footer><i><em style={{ width: `${stay.progress}%` }} /></i><strong>БЕСПЛАТНАЯ ОТМЕНА</strong></footer></article>)}</div></section>;
}
...
function Accommodation() {
  ...
  return <div onClickCapture={(event) => { if (event.target instanceof HTMLButtonElement && event.target.textContent === "Отмена") setShowCancellation(true); }}><AccommodationList /></div>;
}
